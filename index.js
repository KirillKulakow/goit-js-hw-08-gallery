...
function nextImg () {
    ...
    imgList.forEach(function(el, index, array) {
        if (el.alt === document.querySelector('.lightbox__image').getAttribute('alt')) {
            if (index === array.length - 1) {return item = array[array.length - 1]}
            item = array[index + 1]
        }
        })
...
function prevImg () {
    ...
    imgList.forEach(function(el, index, array) {
        if (el.alt === document.querySelector('.lightbox__image').getAttribute('alt')) {
            if (index === 0) {return item = array[0]}
            item = array[index - 1]
        }
        }) 
    setActiveLink(item.getAttribute('data-source'), item.getAttribute('alt'))
